...
const PersonalInfo = () => {
  ...
  function handelFilter(event) {
    const newData = recordList.filter(
      (row) =>
        row.name
          .toLowerCase()
  ...
  const columns = [
    {
      name: "Student Name",
      selector: (info) => (
        <Link className="links" to={`/${info.rollnumber}/data`}>
          {" "}
          {info.name}{" "}
        </Link>
      ),
    ...
    },
    {
      name: "Action",
      selector: (info) => (
        <Link className="btn btn-success" to={`/update/${info.id}`}>
          {" "}
          Update{" "}
        </Link>
      ),
    ...
    },
    {
      ...
      selector: (info) => (
        <button
          onClick={(e) => handelSubmit(info.id)}
  ...
  function handelSubmit(id) {
    ...
    if (conf) {
      axios
        .delete("https://mtiet-api-v2.onrender.com/student/" + id)
        .then((res) => {
